 ...
 * devices Mappo's UserService has open.
 *
 ...
 *
 * That's how Mappo prevents the OS-level dual-dispatch when its virtual
 * gamepad and a physical controller would otherwise both reach the game:
 * grab the physical, let only Mappo's virtual gamepad through. Mappo
 ...
 *
 * Lives in the same `libmappo_uinput.so` as the uinput devices — separate
