...
import com.mapo.shizuku.service.MapoInputUserService
...
/**
 * Defensive wrapper around the static `rikka.shizuku.Shizuku` surface. ALL Mapo code
 ...
 *     class itself is still on Mapo's classpath (it's just an API jar), but the
...
class ShizukuFacade @Inject constructor(
    ...
    /**
     * `true` if Mapo holds the Shizuku permission. Caller should consult
    ...
    /**
     * Build the Shizuku [Shizuku.UserServiceArgs] for [MapoInputUserService].
     * Pinned `processNameSuffix` so the service shows as `com.mapo:mapo_input`
     ...
     */
    fun userServiceArgsForMapoInput(): Shizuku.UserServiceArgs {
        val debuggable = (context.applicationInfo.flags and ApplicationInfo.FLAG_DEBUGGABLE) != 0
        return Shizuku.UserServiceArgs(
            ComponentName(context, MapoInputUserService::class.java)
        )
            .daemon(false)
            .processNameSuffix("mapo_input")
            .debuggable(debuggable)
            .version(MapoInputUserService.PROTOCOL_VERSION)
    }
