...
/**
 * Translates a parsed [VdfControllerConfig] into a Mappo-shaped, Room-id-free
 * **import model** ([ImportedConfig]). This is where every "what does this Steam
 * token become in Mappo" decision lives; the mechanical token tables are in
 * [VdfMappings].
 *
 * **Why an intermediate model and not Room entities.** Mappo entities are keyed by
 ...
 *  - `switches` mode (one VDF group → many Mappo single-button sources)
...
object VdfImporter {
            ...
            return preset.sourceBindings.mapNotNull { sb ->
                ...
                if (source == null) {
                    warnings += ImportWarning(
                        ...
                        "Source '${sb.source}' has no Mappo input source (e.g. bundled 'switch' cluster); group ${sb.groupId} skipped.",
                    )
                    return@mapNotNull null
                }
                ImportedPresetGroup(
    ...
    private fun translateGroup(group: VdfGroup, warnings: MutableList<ImportWarning>): ImportedGroup {
        val mode = VdfMappings.bindingMode(group.mode) ?: run {
            warnings += ImportWarning(
                ...
                "Mode '${group.mode}' (group ${group.id}) has no Mappo equivalent; imported as None (silenced).",
            )
            BindingMode.NONE
        }
     ...
     * input. Mappo models it as a dedicated `soft_pull` *sub-input*. So a trigger
    ...
    /** Translates one VDF binding string into a Mappo command. */
    private fun translateCommand(binding: VdfBinding, warnings: MutableList<ImportWarning>): ImportedCommand {
        ...
        return when (binding.verb) {
            ...
            "key_press" -> {
                ...
                if (code == null) warnings += ImportWarning(ImportWarningKind.UNMAPPED_KEY, "Key '$raw' has no Mappo code; kept verbatim.")
                output(BindingOutput.KeyPress(code ?: raw))
            }
            "xinput_button" -> {
                val raw = binding.args.firstOrNull().orEmpty()
                val btn = VdfMappings.xinputButton(raw)
                if (btn == null) warnings += ImportWarning(ImportWarningKind.UNMAPPED_BUTTON, "Gamepad button '$raw' has no Mappo mapping; kept verbatim.")
                output(BindingOutput.XInputButton(btn ?: raw))
            }
            "mouse_button" -> {
                val raw = binding.args.firstOrNull().orEmpty()
                val btn = VdfMappings.mouseButton(raw)
                if (btn == null) warnings += ImportWarning(ImportWarningKind.UNMAPPED_BUTTON, "Mouse button '$raw' has no Mappo mapping; kept verbatim.")
                output(BindingOutput.MouseButton(btn ?: raw))
            }
            "mouse_wheel" -> {
                val raw = binding.args.firstOrNull().orEmpty()
                val dir = VdfMappings.mouseWheel(raw)
                if (dir == null) warnings += ImportWarning(ImportWarningKind.UNMAPPED_BUTTON, "Mouse wheel '$raw' has no Mappo mapping; kept verbatim.")
...
data class ImportedPresetGroup(
    ...
    /** "active" / "inactive" / "modeshift" (Mappo PresetBinding.state). */
    val state: String,
...
data class ImportedInput(
    /** Mappo sub-input key (e.g. `dpad_up`, `full_pull`). */
    val inputKey: String,
