...
/**
 * The VDF-token → Mapo-type translation tables. Pure, side-effect-free, and unit-
...
internal object VdfMappings {
    ...
    // feedback_steam_mode_naming_convention. Tokens with no faithful Mapo mode
    ...
    fun bindingMode(token: String?): BindingMode? = when (token?.lowercase()) {
        ...
        // Steam "Joystick Mouse" + the camera-tuned variant collapse into Mapo's
        ...
        // No faithful Mapo mode yet:
        //  - switches      → one VDF group fans out to many Mapo single-button sources
    ...
    fun inputSource(token: String): InputSource? = when (token.lowercase()) {
        ...
        // "switch" is the bundled switch cluster — Mapo splits it into START /
    ...
    // VDF spells some keys differently from Mapo's RemapInputOptions vocabulary.
    ...
    /** Mapo's keyboard-code vocabulary (mirrors RemapInputOptions). Used to detect
     *  keys that translate to no Mapo code so the importer can warn. */
    private val mapoKeyCodes: Set<String> = setOf(
    ...
    /** Maps a VDF `key_press` token to a Mapo key code. Returns null when no Mapo
    ...
    fun keyCode(vdfKey: String): String? {
        ...
        return renamed.takeIf { it in mapoKeyCodes }
    }
