...
import com.mappo.service.input.modes.acceptsFor
...
data class CompiledActionSet(
     ...
     * "Mappo intercepts and silences" mode. Maintained separately from [inputs]
 ...
 * the source's [com.mappo.service.input.modes.SourceMode] handler. Brick 1 of Phase 6:
...
data class CompiledInput(
     ...
     * mode handler ([SourceMode.evaluate][com.mappo.service.input.modes.SourceMode.evaluate])
...
/**
 * Decoded view of an [com.mappo.data.model.steam.Activator]'s settingsJson, with every
...
fun ControllerConfig.toCompiled(): CompiledConfig {
    ...
    fun compileInputs(presetEntries: List<com.mappo.data.model.steam.PresetEntry>): CompileResult {
        ...
        for (preset in presetEntries) {
            ...
            // never blank). Skip for DEVICE_DEFAULT — that's the "Mappo doesn't
    ...
    fun compileBindingGroup(group: com.mappo.data.model.steam.BindingGroupGraph): CompiledBindingGroup {
    ...
    fun compileModeShifts(graphs: List<com.mappo.data.model.steam.SourceModeShiftGraph>): List<CompiledModeShift> {
