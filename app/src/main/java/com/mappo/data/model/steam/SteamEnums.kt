 ...
 *  - [DEVICE_DEFAULT] — Mappo does NOT intercept; Android's hardware-native input
 *    flows untouched to the foreground app. Mappo-specific concept.
 *  - [NONE] — Mappo intercepts and silences. Direct Steam Input parity.
 *
...
enum class BindingMode {
    ...
    /**
     * Mappo intercepts this source but emits nothing. Direct Steam Input parity
     * for the "None" mode dropdown option — distinct from [DEVICE_DEFAULT]'s
     * pass-through because here Mappo actively consumes the event to silence it.
     */
    NONE,
     ...
     * game receives a virtual analog stick. Mappo runtime ships in Phase 7
     ...
     * Mappo previously (mis-)called `MOUSE_JOYSTICK`. The cursor-feel modes
     ...
     * mode — supersedes the previous Mappo-specific `ABSOLUTE_MOUSE` (which
