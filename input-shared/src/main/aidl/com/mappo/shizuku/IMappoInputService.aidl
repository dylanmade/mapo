...
/**
 * Binder contract for Mappo's Shizuku UserService. The service runs as UID 2000
...
interface IMappoInputService {
    ...
    void registerCallback(IMappoInputCallback cb) = 2;
    void unregisterCallback(IMappoInputCallback cb) = 3;
     ...
     * race with Mappo's virtual gamepad. Released when no such mode is
     * configured.
     *
     * Touchscreens + unclassified devices are deliberately NOT grabbed —
     * screen touch must keep working for system UI gestures and Mappo's
     * own overlay clicks.
     */
