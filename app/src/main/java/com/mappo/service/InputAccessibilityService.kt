...
import com.mappo.service.shizuku.ShizukuKeyInjector
import com.mappo.service.shizuku.ShizukuMotionCoordinator
import com.mappo.service.shizuku.ShizukuMouseInjector
import com.mappo.service.shizuku.ShizukuStylusInjector
...
class InputAccessibilityService : AccessibilityService(), InputSink {
    ...
    companion object {
        ...
        /**
         * Name prefix of Mappo's own virtual uinput devices (gamepad / mouse / stylus —
         * all "Mappo Virtual …"). Key events from these are skipped in [onKeyEvent] to
         * break the MVG-output → onKeyEvent → re-fire feedback loop. Mirrors the
         * `MAPPO_VIRTUAL_DEVICE_PREFIX` the Shizuku UserService uses to skip them on the
         * /dev/input read side.
         */
     ...
     * (or focused) application window belongs to something other than Mappo. Falls
     ...
     *      non-Mappo foreground report) — strongest signal of user intent.
     *   2. Else: any non-Mappo non-system TYPE_APPLICATION on a non-default display.
     *   3. Else: any non-Mappo non-system TYPE_APPLICATION on the default display.
     *
    ...
    override fun queryPrimaryDisplayForegroundPackage(): String? {
        ...
        // when the user is looking at Mappo on the bottom screen, the primary display's
    ...
    override fun onKeyEvent(event: KeyEvent): Boolean {
        // Feedback-loop guard: ignore key events that originate from Mappo's OWN virtual
        // gamepad (or any "Mappo Virtual …" uinput device). When a binding outputs a
        ...
        // as its controller) — we simply don't re-process Mappo's own output.
        val sourceDeviceName = event.device?.name
        if (sourceDeviceName != null && sourceDeviceName.startsWith(MAPPO_VIRTUAL_DEVICE_PREFIX)) {
            return false
        }
     ...
     * [invalidateDisplaySizeCache]. Mappo's target hardware is landscape-locked
     ...
     * event to the default (top) display, where Mappo, not the game, has focus.
     */
