 ...
 *
 * **Struct layout** (arm64 Android, the only Mappo target):
 * ```
 * struct input_event {
...
object LinuxInputConstants {
    ...
    // Used by :app to call `injectMouseButton(btnCode, pressed)` on Mappo's
     ...
     * Odin 2 / Mini, Anbernic, Retroid — Mappo's target devices). Verified on
